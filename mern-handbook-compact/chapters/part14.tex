...
This chapter wires Chapters 9-13 into a full CRUD feature for the Task resource: model, routes, controller, service, and the React pieces that consume the API.
...
const notFound = () => { const e = new Error("Task not found"); e.statusCode = 404; throw e; };

const createTask = async (data, userId) => Task.create({ ...data, user: userId });
...
const updateTask = async (id, userId, data) => {
  const task = await Task.findOneAndUpdate({ _id: id, user: userId }, data, { new: true, runValidators: true });
  if (!task) notFound();
  return task;
};
...
const createTask = async (req, res, next) => {
  try { res.status(201).json(await taskService.createTask(req.body, req.user.id)); }
  catch (err) { next(err); }
};

const getTasks = async (req, res, next) => {
  try { res.status(200).json(await taskService.getTasks(req.user.id, req.query)); }
  catch (err) { next(err); }
};

const getTaskById = async (req, res, next) => {
  try { res.status(200).json(await taskService.getTaskById(req.params.id, req.user.id)); }
  catch (err) { next(err); }
};

const updateTask = async (req, res, next) => {
  try { res.status(200).json(await taskService.updateTask(req.params.id, req.user.id, req.body)); }
  catch (err) { next(err); }
};
...
const TaskForm = ({ existingTask, onSaved }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      if (existingTask) await updateTask(existingTask._id, form);
...
const TaskList = () => {
  ...
  return (
    <div className="task-list">
      {tasks.map((task) => <TaskCard key={task._id} task={task} onDelete={() => handleDelete(task._id)} />)}
    </div>
  );
};
...
\begin{verbatim}
TaskForm submit -> createTask(form) -> POST /api/tasks (axios, withCredentials)
  -> protect middleware (verifies cookie, sets req.user)
  -> task.controller.createTask (reads req.body, calls service)
  -> task.service.createTask (Task.create({...data, user: userId}))
  -> MongoDB insert -> 201 response -> onSaved() -> TaskList reloads
\end{verbatim}
\end{diagrambox}

\begin{diagrambox}[Delete Task Flow]
\begin{verbatim}
TaskCard "Delete" click -> deleteTask(id) -> DELETE /api/tasks/:id
...
Every service function scopes queries with \texttt{user: userId}. Without this, any authenticated user could read, update, or delete another user's tasks by guessing an \texttt{\_id}.
\end{warnbox}
